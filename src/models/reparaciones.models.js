...
const selectReparacionesByMecanico = async (usuarioId) => {
    const [result] = await db.query(
        "select estado, fecha_ingreso, fecha_finalizacion,estado,presupuesto,precio_total, usuarios.nombre, usuarios.apellidos,usuarios.telefono, reparaciones.id, usuarios_id from reparaciones join usuarios on usuarios_id = usuarios.id where usuarios.id =?",
        [usuarioId],
    );
    return result;
};
const selectVehiculoByReparacion = async (reparacionId) => {
    const [result] = await db.query(
        "select vehiculos.matricula, vehiculos.marca,vehiculos.modelo,vehiculos.fecha_matriculacion, vehiculos.fecha_entrada, vehiculos.fecha_salida from vehiculos join reparaciones on vehiculos_id = vehiculos.id where reparaciones.id= ?",
        [reparacionId],
    );
    return result[0];
};
